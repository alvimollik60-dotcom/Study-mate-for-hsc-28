...
// --- ৮০ সেকেন্ড পর পর ফায়ারবেসে ডেটা পাঠানোর গ্লোবাল কাউন্টার ---
let firebaseSyncCounter = 0; 
...
// --- অ্যাপ বন্ধ থাকলেও ব্যাকগ্রাউন্ডে ক্যালকুলেশন করার রিকভারি ফাংশন ---
function recoverDeadTime() {
    const wasTimerRunning = localStorage.getItem('was_timer_running') === 'true';
    const wasSubTimerRunning = localStorage.getItem('was_sub_timer_running') === 'true';
    
    if (wasTimerRunning || wasSubTimerRunning) {
        const lastActive = parseInt(localStorage.getItem('last_active_timestamp'));
        const mainSessionSaved = parseInt(localStorage.getItem('main_timer_session_seconds')) || 0;
        const subSessionSaved = parseInt(localStorage.getItem('subject_timer_session_seconds')) || 0;
        ...
        if (lastActive) {
            ...
            if (elapsedSeconds > 0) {
                // ২ ঘণ্টার লিমিট চেক (7200 সেকেণ্ড)
                let maxAllowedMain = 7200 - mainSessionSaved;
                let maxAllowedSub = 7200 - subSessionSaved;
                
                let actualElapsedMain = wasTimerRunning ? Math.min(elapsedSeconds, maxAllowedMain) : 0;
                let actualElapsedSub = wasSubTimerRunning ? Math.min(elapsedSeconds, maxAllowedSub) : 0;
                ...
                if (wasTimerRunning && actualElapsedMain > 0) {
                    userDailyLogs[todayStr][currentActivity] = (userDailyLogs[todayStr][currentActivity] || 0) + actualElapsedMain;
                    mainTimerSessionSeconds = mainSessionSaved + actualElapsedMain;
                    localStorage.setItem('main_timer_session_seconds', mainTimerSessionSeconds.toString());
                }
                
                if (wasSubTimerRunning && actualElapsedSub > 0) {
                    userSubjectDailyLogs[todayStr][currentTrackingSubject] = (userSubjectDailyLogs[todayStr][currentTrackingSubject] || 0) + actualElapsedSub;
                    subjectTimerSessionSeconds = subSessionSaved + actualElapsedSub;
                    localStorage.setItem('subject_timer_session_seconds', subjectTimerSessionSeconds.toString());
                    
                    if (currentActivity !== 'Self Study' && wasTimerRunning) {
                        userDailyLogs[todayStr]['Self Study'] = (userDailyLogs[todayStr]['Self Study'] || 0) + actualElapsedSub;
                    }
                }
                
                // জাস্ট ২ ঘণ্টা পার হয়ে গেলে স্টেট বন্ধ করে দেবো
                if (mainTimerSessionSeconds >= 7200) {
                    localStorage.setItem('was_timer_running', 'false');
                    mainTimerSessionSeconds = 0;
                    localStorage.setItem('main_timer_session_seconds', '0');
                }
                if (subjectTimerSessionSeconds >= 7200) {
                    localStorage.setItem('was_sub_timer_running', 'false');
                    subjectTimerSessionSeconds = 0;
                    localStorage.setItem('subject_timer_session_seconds', '0');
                }
...
// নোডিং লজিকটিকে একটি ফাংশনে নিয়ে আসা হলো কাজের সুবিধার্থে
function proceedAppLoading() {
    ...
    // টাইমার স্টেট রিকভারি
    const savedActivity = localStorage.getItem('current_activity_name');
    const savedSubActivity = localStorage.getItem('current_tracking_subject');
    
    if (savedActivity) currentActivity = savedActivity;
    if (savedSubActivity) currentTrackingSubject = savedSubActivity;

    loadUserData().then(() => {
        // অ্যাপ কেটে দেওয়ার পর পুনরায় ওপেন করলে ডেড টাইম রিকভারি করা হবে
        recoverDeadTime();
        ...
        if (wasTimerRunning) {
            mainTimerSessionSeconds = parseInt(localStorage.getItem('main_timer_session_seconds')) || 0;
            startTimer();
        }
        if (wasSubTimerRunning) {
            subjectTimerSessionSeconds = parseInt(localStorage.getItem('subject_timer_session_seconds')) || 0;
            isSubjectTimerRunning = true;
            const subBtn = document.getElementById('subject-timer-toggle-btn');
            if (subBtn) {
                subBtn.innerText = "সাবজেক্ট স্টাডি শেষ করুন";
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // --- অটো-লগইন লজিক (আপডেটেড কিক-আউট সিকিউরিটি চেক সহ) ---
    const savedUser = localStorage.getItem('study_mate_logged_user');
    if (savedUser) {
        currentUser = JSON.parse(savedUser);
        
        // সিকিউরিটি চেক: ইউজারকে অ্যাডমিন কিক দিয়েছে কি না ক্লাউড থেকে ভেরিফাই করা
        if (window.fbFirestore && window.firebaseDb && currentUser.role !== 'admin') {
            const userDocRef = window.fbFirestore.doc(window.firebaseDb, "users", currentUser.uid || "unknown");
            window.fbFirestore.getDoc(userDocRef).then((userSnap) => {
                if (!userSnap.exists()) {
                    alert("আপনার অ্যাকাউন্টটি অ্যাডমিন দ্বারা বাতিল বা কিক করা হয়েছে!");
                    logout();
                    return;
                }
                proceedAppLoading();
...
window.addEventListener('beforeunload', () => {
    ...
    if (currentUser && window.fbFirestore && window.firebaseDb) {
        ...
        window.fbFirestore.setDoc(liveDocRef, {
            [todayStr]: {
                'currentStatus': isRunning ? `${currentActivity} করছেন (ব্যাকগ্রাউন্ড)` : 'অফলাইন / বের হয়ে গেছেন',
...
// --- ২. থিম ইঞ্জিন কালার প্রোভাইডার লজিক (Firebase Real-time Supported) ---
...
async function saveThemeSettings() {
    ...
    const cardVal = document.getElementById('color-card').value;
    const btnVal = document.getElementById('color-btn').value;
    const textVal = document.getElementById('color-text').value;

    try {
        if (window.fbFirestore && window.firebaseDb) {
            await window.fbFirestore.setDoc(window.fbFirestore.doc(window.firebaseDb, "settings", "global_theme"), {
                bg: bgVal,
                card: cardVal,
                btn: btnVal,
                text: textVal,
            ...
            alert('থিম সেটিংস সফলভাবে ক্লাউডে আপডেট করা হয়েছে এবং সবার জন্য পরিবর্তন হবে!');
        } else {
            localStorage.setItem('theme_color_bg', bgVal);
            localStorage.setItem('theme_color_card', cardVal);
            localStorage.setItem('theme_color_btn', btnVal);
            localStorage.setItem('theme_color_text', textVal);
            applySavedTheme();
            alert('লোকাল স্টোরেজে আপডেট করা হয়েছে!');
        }
    } catch (error) {
        console.error("Theme save error:", error);
        alert("থিম ক্লাউডে সেভ করতে সমস্যা হয়েছে।");
    }
}

// --- ৩. নোটিউট ক্লাউড সিঙ্ক লজিক ---
...
// --- ৪. ফায়ারবেসে গ্লোবাল রিয়েল-টাইম লিসেনারস (Theme Sync Included) ---
function initFirebaseGlobalListeners() {
    if(!window.fbFirestore || !window.firebaseDb) return;

    window.fbFirestore.onSnapshot(window.fbFirestore.doc(window.firebaseDb, "settings", "global_theme"), (docSnap) => {
        if (docSnap.exists()) {
            const themeData = docSnap.data();
            
            localStorage.setItem('theme_color_bg', themeData.bg);
            localStorage.setItem('theme_color_card', themeData.card);
            localStorage.setItem('theme_color_btn', themeData.btn);
            localStorage.setItem('theme_color_text', themeData.text);

            document.documentElement.style.setProperty('--theme-bg', themeData.bg);
            document.documentElement.style.setProperty('--theme-card-bg', themeData.card);
            document.documentElement.style.setProperty('--theme-btn-bg', themeData.btn);
            document.documentElement.style.setProperty('--theme-text-color', themeData.text);

            if (document.getElementById('color-bg')) document.getElementById('color-bg').value = themeData.bg;
            if (document.getElementById('color-card')) document.getElementById('color-card').value = themeData.card;
            if (document.getElementById('color-btn')) document.getElementById('color-btn').value = themeData.btn;
            if (document.getElementById('color-text')) document.getElementById('color-text').value = themeData.text;
        }
    });
...
// --- ६. অথেনটিকেশন ও কোর অ্যাপ্লিকেশন রাউটিং SYSTEM ---
async function login() {
    ...
    try {
        ...
        localStorage.setItem('study_mate_logged_user', JSON.stringify(currentUser));
...
function logout() {
    ...
    localStorage.removeItem('study_mate_logged_user');
    localStorage.removeItem('was_timer_running');
    localStorage.removeItem('was_sub_timer_running');
    localStorage.removeItem('last_active_timestamp');
    localStorage.removeItem('current_activity_name');
    localStorage.removeItem('current_tracking_subject');
    localStorage.removeItem('main_timer_session_seconds');
    localStorage.removeItem('subject_timer_session_seconds');
...
async function loadUserData() {
    ...
    try {
        if(window.fbFirestore && window.firebaseDb) {
            // --- ৩ নম্বর প্রোটেকশন: ব্যাকগ্রাউন্ড সিকিউরিটি চেক ---
            if (currentUser.role !== 'admin') {
                const checkRef = window.fbFirestore.doc(window.firebaseDb, "users", currentUser.uid || "unknown");
                const checkSnap = await window.fbFirestore.getDoc(checkRef);
                if (!checkSnap.exists()) {
                    logout();
                    return;
                }
    ...
    if(!currentTrackingSubject) currentTrackingSubject = fixedSubjects[0];
...
function startTimer() {
    ...
    localStorage.setItem('was_timer_running', 'true');
    localStorage.setItem('current_activity_name', currentActivity);
...
function handleTimerTick() {
    ...
    if (isSubjectTimerRunning) {
        ...
        if (currentActivity !== 'Self Study' && isRunning) {
            userDailyLogs[todayStr]['Self Study'] = (userDailyLogs[todayStr]['Self Study'] || 0) + 1;
        }
    }

    localStorage.setItem('last_active_timestamp', Date.now().toString());
    localStorage.setItem('was_timer_running', isRunning.toString());
    localStorage.setItem('was_sub_timer_running', isSubjectTimerRunning.toString());
    localStorage.setItem('current_activity_name', currentActivity);
    localStorage.setItem('current_tracking_subject', currentTrackingSubject);

    if (mainTimerSessionSeconds >= 7200) { 
        stopTimer();
        alert("আপনার মেইন টাইমারটি একটানা ২ ঘণ্টা অতিক্রম করায় অটোমেটিক বন্ধ করা হয়েছে।");
        return;
    }
    
    if (subjectTimerSessionSeconds >= 7200) {
        stopSubjectTimer();
        alert("আপনার সাবজেক্ট টাইমারটি একটানা ২ ঘণ্টা অতিক্রম করায় অটোমেটিক বন্ধ করা হয়েছে।");
        return;
    }
    ...
    // --- 🆕 ৮০ সেকেন্ড পর পর ফায়ারবেসে ডেটা পাঠানোর লজিক ---
    firebaseSyncCounter++;
    if (firebaseSyncCounter >= 80) {
        syncTimeWithFirebaseCloud(todayStr);
        firebaseSyncCounter = 0; // কাউন্টার রিসেট
        console.log("৮০ সেকেন্ড পূর্ণ হয়েছে, ফায়ারবেসে ডেটা পাঠানো হয়েছে।");
    }
}
...
function stopTimer() {
    ...
    // টাইমার স্টপ করার সাথে সাথে লেটেস্ট ডাটা পুশ করে দেওয়া এবং কাউন্টার রিসেট করা
...
// --- ८. পাবলিক স্টাডি ড্যাশবোর্ড ইঞ্জিন ---
...
// --- ९. সাবজেক্ট ট্র্যাকার ও পোমোডোরো লজিক ---
...
function toggleSubjectTimer() {
    if(isSubjectTimerRunning) {
        ...
        isSubjectTimerRunning = true;
        localStorage.setItem('was_sub_timer_running', 'true');
        localStorage.setItem('current_tracking_subject', currentTrackingSubject);
...
async function uploadResourceLinksToFirebase() {
    ...
    } catch(e) { console.error("Resource upload error:", e); }
}
...
// প্রোটেকশন চেক এভয়েড করার জন্য কনসোল ক্লিয়ার
...
// ডাইনামিক ইন্টারফেস লিংক রেন্ডারিং
...
// --- ১ ও ২ নম্বর প্রোটেকশন: কিক-আউট লজিক ---
async function deleteLiveUser(docId, targetUsername) {
    ...
    if (confirm(`আপনি কি নিশ্চিতভাবে "${targetUsername}" ইউজারকে সম্পূর্ণ কিক করতে চান?`)) {
        try {
            if(window.fbFirestore && window.firebaseDb) {
                // ১. ইউজার কালেকশন থেকে সম্পূর্ণ ডিলিট
                await window.fbFirestore.deleteDoc(window.fbFirestore.doc(window.firebaseDb, "users", docId));
                
                // ২. লাইভ অ্যাক্টিভিটি কালেকশন থেকেও ডিলিট
                await window.fbFirestore.deleteDoc(window.fbFirestore.doc(window.firebaseDb, "liveActivity", targetUsername));
                
                renderUserTable();
                alert(`"${targetUsername}" সফলভাবে কিকড এবং ড্যাশবোর্ড থেকে রিমুভ হয়েছে।`);
            }
        } catch (e) { console.error(e); }
    }
}
...
// --- ১০. ফোন লক বা ব্যাকগ্রাউন্ড থেকে অন করার ইমার্জেন্সি রি-সিঙ্ক ব্যাকআপ লেয়ার ---
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
        recoverDeadTime();
        
        const wasTimerRunning = localStorage.getItem('was_timer_running') === 'true';
        if (wasTimerRunning && !isRunning) {
            startTimer();
        }
